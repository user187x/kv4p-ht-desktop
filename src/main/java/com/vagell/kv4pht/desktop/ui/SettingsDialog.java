...
final class SettingsDialog {
    ...
    static void show(Component parent, AppDatabase db, RadioAudioService radio) {
        ...
        JCheckBox emphasis = new JCheckBox("Pre- & De-emphasis", "1".equals(db.getSetting("emphasis", "1")));
        ...
        p.add(new JLabel("Bandwidth:"));       p.add(bandwidth);
        p.add(new JLabel("Max TX frequency:"));p.add(maxFreq);
        p.add(new JLabel("Mic gain boost:"));  p.add(micGain);
        p.add(new JLabel("Callsign:"));        p.add(callsign);
        p.add(new JLabel("Filters:"));         p.add(emphasis);
        p.add(new JLabel(""));                 p.add(highpass);
        p.add(new JLabel(""));                 p.add(lowpass);

        int res = JOptionPane.showConfirmDialog(parent, p, "Settings",
