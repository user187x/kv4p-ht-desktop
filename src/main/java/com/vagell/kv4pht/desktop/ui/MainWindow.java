...
public class MainWindow extends JFrame {
    ...
    private void buildUi() {
        ...
        squelchSlider.addChangeListener(e -> {
        ...
        memButtons.add(addBtn); memButtons.add(editBtn); memButtons.add(delBtn);
    ...
    private void wireRadio() {
        radio.setCallbacks(new RadioAudioServiceCallbacks() {
            ...
            @Override public void radioMissing() {
                SwingUtilities.invokeLater(() ->
            ...
            @Override public void missingFirmware() {
                SwingUtilities.invokeLater(() -> {
            ...
            @Override public void sMeterUpdate(int v) {
                SwingUtilities.invokeLater(() -> {
                    sMeter.setValue(v);
                    sMeter.setString("S" + v);
                });
            }
            @Override public void scannedToMemory(int memoryId) {
    ...
    private void addMemory() {
        ...
        if (m != null) { db.insertMemory(m); refreshMemories(); }
    }
    ...
    private void editMemory() {
        ...
        if (m != null) { db.updateMemory(m); refreshMemories(); }
    }

    private void deleteMemory() {
        ChannelMemory sel = memList.getSelectedValue();
        if (sel == null) return;
        if (JOptionPane.showConfirmDialog(this, "Delete \"" + sel + "\"?",
    ...
    private static final class PortItem {
        ...
        PortItem(SerialPort p) { this.port = p; }
