...
public class FirmwareDialog extends JDialog {
    ...
    private final JLabel status = new JLabel("Ready to flash firmware v" + FirmwareFlasher.PACKAGED_FIRMWARE_VER + ".");
    ...
    private void buildUi() {
        ...
        closeButton.addActionListener(e -> { if (!busy) dispose(); });
    ...
    private void startFlashing() {
        ...
        new SwingWorker<Boolean, Object[]>() {
            ...
            @Override protected Boolean doInBackground() {
                flasher.flashFirmware(io, new FirmwareFlasher.Callback() {
                    @Override public void connectedToBootloader() {
                        publish(new Object[]{"status", "Connected to bootloader. Flashing..."});
                    }
                    @Override public void reportProgress(int percent) {
                        publish(new Object[]{"progress", percent});
                    }
                    @Override public void info(String message) {
                        publish(new Object[]{"status", message});
                    }
                    @Override public void doneFlashing(boolean ok) {
                        success = ok;
                    }
                });
                return success;
