...
This class is a faithful, dependency-free port of the protocol/DSP logic that
lived inside the Android RadioAudioService. Keeping it pure Java makes it unit
testable and guarantees the desktop app speaks byte-for-byte the same protocol
to the ESP32 firmware as the original app.
*/
...
public final class RadioProtocol {
    ...
    // ---- Wire constants (must match ESP32 firmware) ----
    public static final byte[] COMMAND_DELIMITER =
            {(byte) 0xFF, (byte) 0x00, (byte) 0xFF, (byte) 0x00,
             (byte) 0xFF, (byte) 0x00, (byte) 0xFF, (byte) 0x00};
    public static final byte COMMAND_SMETER_REPORT = 0x53; // ASCII 'S'

    /** Midpoint of unsigned-8-bit PCM == silence. (byte)-128 == 0x80 == 128 unsigned. */
    public static final byte SILENT_BYTE = -128;

    public static final int AUDIO_SAMPLE_RATE = 22050;

    public static final int SERIAL_BAUD = 230400;

    public static final String VERSION_PREFIX = "VERSION";
    public static final int VERSION_LENGTH = 8;

    // USB identifiers of the supported ESP32 board (matches the Android app).
    public static final int ESP32_VENDOR_ID = 4292;   // 0x10C4 (Silicon Labs CP210x)
    public static final int ESP32_PRODUCT_ID = 60000;  // 0xEA60

    // ---- CTCSS tone index table (string -> firmware index) ----
    ...
    /** Tone string -> zero padded 2-digit firmware index, e.g. "82.5" -> "06". */
    ...
    /**
     * Normalizes a user-entered frequency to the "xxx.xxxx" form the radio module
     * expects, clamped to the module's supported 2 m range (134-174 MHz).
     */
    public static String makeSafe2MFreq(String strFreq) {
        ...
        // Handle "1467"/"14670" meaning "146.7".
    ...
    /** Applies a repeater offset (+/-0.600 MHz) to derive the TX frequency. */
    ...
    /** Offset constants, mirrored from ChannelMemory for use without a data dep. */
    ...
    /** Builds a bare command frame: DELIMITER + cmdByte. */
    ...
    /** Builds a command frame with ASCII params: DELIMITER + cmdByte + params. */
    ...
    /** Builds the TUNE_TO param string for simplex operation. */
    ...
    /** Builds the TUNE_TO param string for a memory (with offset and tone). */
    ...
    /** Builds the FILTERS param string. */
    ...
    // ---- S-meter scaling (raw 0-255 -> S1..S9), ported verbatim ----
    ...
    /** Applies software mic gain to unsigned-8-bit PCM, with clipping. */
    ...
    // ======================================================================
    // RX stream de-framer: separates raw audio bytes from embedded command
    // frames in the ESP32 -> host stream. Stateful (keeps a leftover buffer
    // across calls). Ported from RadioAudioService.extractAudioAndHandleCommands.
    // ======================================================================
    ...
    public static final class RxParser {
        ...
        public synchronized byte[] parse(byte[] newData, CommandHandler handler) {
            ...
            while (true) {
                ...
                int neededBeforeParams = COMMAND_DELIMITER.length + 2; // cmd + paramLen
    ...
    /** First occurrence of pattern in data at/after start, or -1. */
    ...
    /** How many trailing bytes might be the start of a not-yet-complete delimiter. */
